...
function krpanoplugin() {
  ...
  function generateVideoList() {
    ...
    const allSections = bsvl.concat(asl);  // Combine both video lists if needed
    if (allSections.reduce((sum, current) => sum += current.videos.length, 0) >= 16)
    {
       let vid_list = '<div id="vid_list" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; text-align: center;">';
   
       // Assuming 'bsvl' and 'asl' contain different sections of videos
       const allSections = bsvl.concat(asl);  // Combine both video lists if needed
       allSections.forEach((section, index) => {
           vid_list += `<div><b><span style="font-size:1.4em">${section.title}</span></b><br>`;
