...
function krpanoplugin() {
    ...
    local.registerplugin = function (krpanointerface, pluginpath, pluginobject) {
        ...
        plugin.registerattribute("show_floor_menu_dynamic", show_floor_menu_dynamic);
    ...
    function show_video(n) {
       ...
       try{ var player = new Vimeo.Player('vimeo-player', {
            id: videoLibrary[videoData.id], // Assuming videos[0] contains the Vimeo video ID
            width: 640,
            autoplay: true
        });
    } catch(err) {
        var player = new Vimeo.Player('vimeo-player', {
            id: videoData.id, // Assuming videos[0] contains the Vimeo video ID
            width: 640,
            autoplay: true
        });
    }
    ...
    function show_floor_menu_dynamic(html) {
        hide_overlay();
        current_type = "html";
        plugin.visible = true;
        ...
        doc.querySelectorAll(".floor_link").forEach((link) => {
          ...
          if (
            ...
            if (
              organizedLinks.length > 0 &&
              organizedLinks[organizedLinks.length - 1].floor == floorNum
            ) {
              organizedLinks[organizedLinks.length - 1].links.push({
                pos: Positions.Left,
                html: link.outerHTML,
              });
            } else {
              organizedLinks.push({
                floor: floorNum,
                links: [{ pos: Positions.Left, html: link.outerHTML }],
              });
            }
          } else if (
            text.includes("East") ||
            ...
            if (
              ...
              organizedLinks.push({
                floor: floorNum,
                links: [{ pos: Positions.Right, html: link.outerHTML }],
              });
            }
          } else {
            organizedLinks.push({
              floor: floorNum,
              links: [{ pos: Positions.Whole, html: link.outerHTML }],
            });
          }
        });
  
        //Makes everything whole positions if only one side is used
        if (
          !(
            organizedLinks.some((ol) =>
              ol.links.some((l) => l.pos == Positions.Left)
            ) &&
            organizedLinks.some((ol) =>
              ol.links.some((l) => l.pos == Positions.Right)
            )
          )
        ) {
          organizedLinks.forEach((ol) =>
            ol.links.forEach((l) => (l.pos = Positions.Whole))
          );
        }
  
        let constructedTable = "";
  
        organizedLinks.forEach((linkRow, i) => {
          if (linkRow.links.length > 1) {
            constructedTable += `<tr>
                      <td style="font-size: 1em;vertical-align:bottom;text-align:center;padding: 0 1.5em 0 0;text-align: right;width:205px;">
                          ${
                            linkRow.links.find((l) => l.pos == Positions.Left)
                              .html
                          }
                      </td>
                      <td style="font-size: 1em;vertical-align:bottom;text-align:center;padding: 0 0 0 1.5em;text-align: left;width:205px;">
                          ${
                            linkRow.links.find((l) => l.pos == Positions.Right)
                              .html
                          }
                      </td>
                  </tr>`;
          } else {
            if (linkRow.links[0].pos == Positions.Whole) {
              constructedTable += `<tr>
                          <td colspan="4" style="font-size: 1em;vertical-align:bottom;text-align:center;padding: 0;">
                              ${linkRow.links[0].html}
                          </td>
                      </tr>`;
            } else if (linkRow.links[0].pos == Positions.Left) {
              constructedTable += `<tr>
                          <td style="font-size: 1em;vertical-align:bottom;text-align:center;padding: 0 1.5em 0 0;text-align: right;width:205px;">
                              ${linkRow.links[0].html}
                          </td>
                          <td style="font-size: 1em;vertical-align:bottom;text-align:center;padding: 0 0 0 1.5em;width:205px;">
                          </td>
                      </tr>`;
            } else if (linkRow.links[0].pos == Positions.Right) {
              constructedTable += `<tr>
                          <td style="font-size: 1em;vertical-align:bottom;text-align:center;padding: 0 1.5em 0 0;width:205px;">
                          </td>
                          <td style="font-size: 1em;vertical-align:bottom;text-align:center;padding: 0 0 0 1.5em;text-align: left;width:205px;">
                              ${linkRow.links[0].html}
                          </td>
        ...
        // Construct new HTML with categorized links
        const newHtml = `
  <div class="floor-menu-container flex-container animated fadeIn bg-overlay margin:0">
      <table style="border-spacing: 2px;">
          ${constructedTable}
          <tr>
          <td colspan="4" style="font-size: 1em;vertical-align:bottom;text-align:center;"><img src="../${app_version}/skin/ak/close_but.png" id="close_but" /></td>
          </tr>
      </table>       
  </div>
  `;
    ...
    function show_item_lib_hcn(scene, floor_id) {
    ...
    function show_item_lib(scene, floor_id) {
        ...
        let closest_div = document.querySelectorAll('.item-closest-container');
        let ov = krpano.get('plugin[scene]');
        let floorspots = ov.get_path_data(true);
    
        // Populate stairs data
        let itemStairs = document.getElementById("item-stairs");
        for (let gs_id in floorspots) {
            let current_floorspot = floorspots[gs_id];
            if (current_floorspot.stairs.length > 0) {
                for (let stair of current_floorspot.stairs) {
                    itemStairs.innerHTML += "<a style='padding:.5em;font-size:1.3em' data-label='" + gs_id + "' data-item='stairwell' class='item-lib-link'>" + stair.toUpperCase() + " </a>";
                }
            }
        }
